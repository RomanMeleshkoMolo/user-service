...
async function seed() {
  try {
    ...
    // Удаляем как новый формат "Київ, Київська область, Україна",
    // так и старый формат "Київ" (только название города)
    const OLD_UA_CITIES = UA_CITIES.map(c => c.split(',')[0].trim());
    const deleted = await User.deleteMany({
      userLocation: { $in: [...UA_CITIES, ...OLD_UA_CITIES] },
      email: { $exists: false },
    ...
    const users = [];
    for (let i = 0; i < 75; i++) users.push(buildUser(i, 'female'));
    for (let i = 0; i < 75; i++) users.push(buildUser(i, 'male'));
